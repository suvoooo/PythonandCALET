...
sig1c=m.contour("ce","ratse",1.0)
print "calculating 2 sigma countour p5 vs p6"
sig2c=m.contour("ce","ratse",2.0)
...
def ploterrorcontours(contourlist,param1,param2):
	...
	atandict = {}
        for xy in cont:
		newval = math.atan2(xy[1]-C_p,xy[0]-C_e)
		atandict[newval] = xy
	for point in sorted(atandict.keys()):
		xy=atandict[point]
    		#lte=1/((mltfac)*xy[0])
...
ploterrorcontours(contourlist,"ce","ratse")
